# CDAC Interview Questions — Answered Using PlatterOps
...
Every entity in PlatterOps (like `MenuItem`, `Order`, `User`) uses private fields with public getters/setters via Lombok's `@Getter` and `@Setter`. No external class can directly touch the field — they go through controlled methods.
...
In PlatterOps, every Filter overrides `doFilterInternal()` from `OncePerRequestFilter`. At runtime, Spring decides which filter to call based on the actual object in the chain:
...
In PlatterOps: `Order` and `OrderItem` — an `OrderItem` only makes sense inside an `Order`. If the order is deleted (soft-deleted), the items are meaningless without it. This is composition.
...
In PlatterOps: `Restaurant` and `User`. A `User` (staff member) belongs to a restaurant, but the `User` entity exists independently — if the restaurant closes, the user account could still exist. The `User` holds a `tenantId` reference but isn't destroyed with the restaurant.
...
In PlatterOps, lambda expressions (which implement functional interfaces) are used throughout:
...
In PlatterOps, Spring beans are singletons by default. When Spring creates `OrderService`, it creates it once and reuses that same instance for every request:
...
In PlatterOps, Redis serializes Java objects to JSON when caching:
...
**In PlatterOps:** JWT secret, database URLs, Redis host — all stored as String configuration. Their immutability means no code can accidentally mutate them after they're set.
...
**In PlatterOps:** `InvoicePdfGenerator` builds invoice text dynamically:
...
**Warning:** Unboxing a `null` `Integer` causes a `NullPointerException`. This is why PlatterOps stores `price` as primitive `int` (can never be null) rather than `Integer`.
...
Yes — PlatterOps's `GlobalExceptionHandler` is essentially a set of catch blocks for the whole application:
...
**In PlatterOps:** `@Transactional` is the equivalent of finally for database work. Even if an exception is thrown inside `placeOrder()`, Spring rolls back the transaction — it's like a built-in finally that cleans up database state.
...
| Collection | Where Used in PlatterOps | Why |
|---|---|---|
...
**In PlatterOps:** `HashMap` is used everywhere because:
...
**In PlatterOps:** When sorting menu items by `displayOrder`:
...
**In PlatterOps — AnalyticsService:**
...
**In PlatterOps — the classic trap:**
...
**In PlatterOps:** `application.yml` has ~30 lines. Spring Boot auto-configures: HikariCP connection pool, Flyway migrations, Redis cache, JPA/Hibernate, Spring Security, and Tomcat — all from those few lines.
...
You can override any auto-configuration by defining your own bean. In PlatterOps, `SecurityConfig` and `CacheConfig` override the defaults with custom behavior.
...
**When PlatterOps accesses `order.getItems()` outside a transaction → LazyInitializationException!**
...
| `@Autowired` | Dependency injection | Injects beans (constructor injection preferred in PlatterOps) |
...
In PlatterOps, `TenantAuthorizationFilter` is a `@Component` — it's not a service or repository, just a Spring-managed filter.
...
**In PlatterOps — `OrderService.placeOrder()`:**
...
| Code | Meaning | Where in PlatterOps |
|---|---|---|
...
- `PATCH /orders/{id}/status` with `CONFIRMED` → idempotent (confirming an already-confirmed order should not change anything — though PlatterOps throws an error for invalid transitions, the *intent* is idempotent)
...
**PlatterOps filter chain (simplified):**
...
```json
{
  ...
  "role": "TENANT_ADMIN",
  "tokenType": "access",    // PlatterOps distinguishes access vs refresh tokens
...
| In PlatterOps | `JwtAuthFilter` validates token → sets SecurityContext | `@PreAuthorize("hasRole('ADMIN')")` checks role |
...
### Q: What is CSRF? How does PlatterOps handle it?
...
**PlatterOps prevention:**
1. **CSRF is disabled** in Spring Security — because PlatterOps uses JWT (not session cookies) for authentication
...
**Never in plain text.** PlatterOps uses BCrypt:
...
**PlatterOps is designed for horizontal scaling:**
...
**If 10,000 customers suddenly hit PlatterOps:** Kubernetes auto-scales (HPA) by adding more backend pod replicas. Redis absorbs the cache load. PostgreSQL connection pool (5-15 per instance) is multiplied across replicas.

---

### Q: What is NoSQL? When would you use it vs PostgreSQL?

**SQL (PostgreSQL in PlatterOps):** Structured data, fixed schema, ACID transactions, relationships via foreign keys.

**NoSQL (e.g., MongoDB, Redis, Cassandra):** Flexible schema, horizontally scalable, no JOINs.

**Why PlatterOps chose PostgreSQL:**
...
**Redis IS used in PlatterOps** — but for caching and pub/sub, not as the primary data store.
...
| Property | Meaning | In PlatterOps |
|---|---|---|
...
PlatterOps: `order_items` is a separate table (not storing items as a comma-separated string in `orders`). Each row is one item.
...
PlatterOps: `OrderItem` stores `tenant_id` denormalized (V23) for performance — technically a violation of 2NF, but a deliberate trade-off. In a strict 2NF design, you'd derive `tenant_id` via a JOIN to `orders`.

**3NF (Third Normal Form):** No transitive dependencies — non-key columns don't depend on other non-key columns.

PlatterOps: `MenuItem` stores `category_id` (FK), not `category_name`. The `name` is derived by joining with `menu_categories`, not stored redundantly. This prevents the inconsistency of a category name being different across menu items.
...
**Non-clustered index:** A separate data structure pointing to rows. PlatterOps's `idx_orders_tenant_status` is non-clustered — the rows stay in their physical order, the index is a separate B-tree.
...
**PlatterOps example (analytics query):**
...
### Q: Find the 2nd highest salary (or in PlatterOps context: 2nd highest order value).
...
**In PlatterOps (V24 — analytics views):**
...
```sql
-- PlatterOps: Find restaurants that exceeded their subscription limit this month
...
| Key | PlatterOps Example | Purpose |
|---|---|---|
...
| **Composite Key** | `(order_id, menu_item_id)` in `order_items` | Could be used — PlatterOps uses a separate UUID PK but has unique constraints on composites |

**Referential integrity in PlatterOps:** If you try to insert an `order_item` with a `menu_item_id` that doesn't exist in `menu_items`, the DB raises a foreign key constraint error — caught by Spring as `DataIntegrityViolationException`.
...
**In PlatterOps:** React uses `key={order.id}` on order cards in the kitchen Kanban:
...
### Q: What is props drilling and how does PlatterOps avoid it?
...
**PlatterOps uses Zustand** — simpler, less boilerplate, same concept:
...
**Why PlatterOps frontend uses TypeScript:**
```tsx
...
In PlatterOps, TypeScript interfaces match the Java DTOs exactly — if the API contract changes, TypeScript immediately highlights all places in the frontend that need updating.
...
**In PlatterOps (k8s/ and docker-compose):**
...
**PlatterOps k8s/ manifests include:**
...
**PlatterOps pipeline (conceptual):**
...
**Branching strategy in PlatterOps:**
...
|---|---|---|---|
| 7 | Application | HTTP, HTTPS, WebSocket | PlatterOps API calls |
...
**Port numbers exist in Layer 4 (Transport layer).** PlatterOps backend listens on port `8080` (TCP). HTTPS is port `443`. The database (PostgreSQL) listens on port `5432`.

---

### Q: What is horizontal scaling vs vertical scaling? (More depth)

**In PlatterOps with Kubernetes:**
...
```
When CPU exceeds 70%, Kubernetes adds pods. When it drops, pods are removed. Works because PlatterOps is **stateless** — any pod can handle any request.
...
> "I built PlatterOps — a multi-tenant restaurant management SaaS platform built with Spring Boot 3.5 and React 19. It allows multiple restaurants to manage their menus, orders, inventory, staff, and analytics from a single platform.
>
...
*All answers reference actual PlatterOps codebase code and decisions.*
